...
**연인·친구가 캐주얼하게 같이 하는 게임.** 두 사람의 게임 실력이 달라도 둘 다 재미있어야 한다.
실력 좋은 플레이어를 위한 도전은 난이도 설정으로 따로 제공한다.

### 핵심 원칙

1. **누가 실수해도 티가 나지 않는다**
   - 한 사람의 실수로 런이 바로 끝나거나 "네 탓"이 되는 구조를 피한다
   - 공유 체력은 넉넉하게, 한 번 맞은 것이 치명적이지 않게 (숫자 체력 100)
   - 협동 보너스는 "잘하면 더 좋다"로 설계하고, "못하면 손해"로 만들지 않는다
2. **시원시원하다**
   - 적이 빨리 쓰러지고, 때리는 맛이 확실하다 (히트스톱·넉백·화면 흔들림·데미지 숫자)
   - 사망 연출은 크게 튕겨나가며 사라지게 → 여러 마리를 쓸어버리는 쾌감
3. **패링·실드는 주가 아니라 보조 수단이다**
