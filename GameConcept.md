...
> 2026-09-20: **디자인 방향 확정** (대상 플레이어·핵심 원칙·역할별 난이도), 숫자 체력 전환.
...
## 디자인 방향 (2026-09-20 확정)

### 누구를 위한 게임인가 (2026-09-23 보강)

**연인·친구가 같이 하는 게임. 단, 캐주얼 전용 게임은 아니다.**

- 목표는 **"쉬워도 도전 의식이 생기는"** 난이도. 배우면 확실히 늘고, 이겼을 때 성취감이 남아야 한다
- 캐주얼은 **선택지(난이도 옵션)**이지 게임 전체의 성격이 아니다.
  기본 난이도에서도 아무 생각 없이 이길 수 있으면 안 된다
- 실력 차이가 나는 두 사람이 같이 즐길 수 있어야 한다 → **역할별 난이도**로 해결
- **"쉽게 만든다"가 아니라 "읽을 수 있게 만든다"**: 적의 공격은 예고되고, 죽은 이유가 납득되어야 한다.
  피할 수 있었는데 못 피한 것은 괜찮지만, 보이지도 않는 공격에 죽는 것은 안 된다

### 핵심 원칙

1. **한 번의 실수가 곧바로 런의 끝이 되지 않는다**
   - 한 사람의 실수로 즉사하거나 "네 탓"이 되는 구조를 피한다. 만회할 여지를 준다
   - 공유 체력은 여러 대 버틸 수 있게 (숫자 체력 100). 단, 계속 맞으면 분명히 죽는다
   - 협동 보너스는 "잘하면 더 좋다"로 설계하고, "못하면 손해"로 만들지 않는다
2. **시원시원하다**
   - 적이 빨리 쓰러지고, 때리는 맛이 확실하다 (히트스톱·넉백·화면 흔들림·데미지 숫자)
   - 사망 연출은 크게 튕겨나가며 사라지게 → 여러 마리를 쓸어버리는 쾌감
3. **공격은 반드시 예고된다 (텔레그래프)** — 2026-09-23 구현
   - 적·보스는 공격 전에 색으로 예고한다: **노랑 = 양파가 막거나 패링 가능 / 빨강 = 막을 수 없음, 피해야 함**
   - 색만 보고 "막을까 / 피할까"를 정할 수 있어야 한다 → 적 기본색은 경고색(빨강·노랑)과 겹치지 않게 그릴 것
   - **예고가 시작되면 그 공격은 확정이다** — 때려도, 도망가도 그대로 발동한다 (취소는 처치뿐).
     공격을 끊는 것이 아니라 **피하거나 막는 것**이 대응 수단 (2026-09-23 결정)
   - 예고 시간은 난이도 옵션으로 조절 (쉬움=길게 / 어려움=짧게)
4. **패링·실드는 주가 아니라 보조 수단이다**
   - 패링을 몰라도 클리어할 수 있어야 한다. 패링은 잘하는 사람이 더 멋지게 하는 선택지
   - 액티브 실드·패링을 강제하는 적/보스 패턴은 만들지 않는다 (예: "패링해야만 뚫리는 보스" 금지)
   - 패시브 실드는 자동으로 공격 한 번을 막아주는 안전망 역할

### 역할별 난이도 (고양이 난이도 / 양파 난이도 따로 선택)

런 시작 시 두 플레이어가 **각자 자기 역할의 난이도**를 고른다. 실력 차이가 나는 두 사람이 같이 즐기기 위한 장치.
세부 수치는 디테일 설계에서 확정 — 아래는 조절 후보.

| 조절 후보 | 고양이 난이도 | 양파 난이도 |
|---|---|---|
| 쉬움 쪽 | 대쉬 무적 시간 증가 / 할퀴기 범위 확대 / 적 추적 보정 | 조준 보정(가까운 적 쪽으로 휘어짐) / 투사체 크기 증가 / 자동 연사 |
| 어려움 쪽 | 대쉬 쿨다운 증가 / 할퀴기 범위 축소 | 조준 보정 없음 / 패링 판정 시간 축소 |
| 공통 영향 | 받는 데미지는 두 난이도의 평균으로 결정 (한 명만 어려움을 골라도 전체가 조금 어려워짐) | |
...
|----------|------|------------|------------|------|
| 1 | **무너진 정원** (지상) | 식물형 — 근접 취약 위주 (Cat 주도) | 따뜻한 초록·노랑 | **영웅 기사** — 방패 돌진형. 돌진을 피하면 빈틈 (패링하면 더 큰 빈틈) |
...
- **숫자 체력 100** (고양이·양파 공유). 하트 방식은 한 대가 너무 무거워 캐주얼 방향과 맞지 않아 폐기
- 적 공격마다 데미지가 다름 (현재 임시값: 슬라임 접촉 10 / 원거리 탄 8 — 밸런스는 M4)
- HUD: 체력바 + "90 / 100" + 깎인 만큼 잠깐 남는 트레일, 그 아래 실드 칸

### 실드 시스템 (2레이어 · 구현 완료)

| 레이어 | 스크립트 | 역할 |
|--------|----------|------|
| 패시브 실드 | `PlayerHealth.cs` | **1칸 = 공격 1회를 데미지와 상관없이 통째로 막음.** 4초 대기 후 2초마다 1칸 자동 회복 |
...
### 타격 피드백 (2026-09-20 구현)

- **데미지 숫자**: 보통=흰색 / 약점(2배↑)=주황 "N!" 크게 / 저항=하늘색 작게 / 면역="BLOCK" 회색.
  고양이 피격=빨강 "-N", 패시브 실드가 막음=하늘색 "BLOCK"
- **타격 강도 3단계** (`HitFeel`): Light=양파 투사체 / Medium=고양이 할퀴기 / Heavy=약점 공격·마지막 일격

  | 강도 | 히트스톱 | 넉백 배율 | 화면 흔들림 |
  |---|---|---|---|
  | Light | 0.03초 | ×0.5 | 없음 (연사라서) |
  | Medium | 0.07초 | ×1.0 | 약하게 |
  | Heavy | 0.12초 | ×1.8 | 강하게 |

- 히트스톱은 Time.timeScale 대신 **맞은 적만 정지** → 다른 플레이어·다른 적의 흐름을 끊지 않음
- 넉백은 공격이 날아온 방향으로. 보스·방패병은 `knockbackResistance`로 덜 밀림
- 사망: 크게 튕겨나가며 작아지고 투명해짐 (0.25초)
- 화면 흔들림은 멀미 옵션으로 끌 수 있게 할 것 (`CameraController.shakeMultiplier`)
- 후보: 소리(타격음·금속음), 파티클 — 사운드 작업(M3) 때
